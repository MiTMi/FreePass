...
- **Extension**: [background.js](FreePassExtension%20Extension/Resources/background.js) calls `sendNativeMessage` on every request to read the current token from the shared keychain via `SafariWebExtensionHandler`. There is intentionally **no** `browser.storage.local` cache — that path was tried and required a `"storage"` permission we don't declare, and the always-fresh token means rotation works trivially without a refresh-on-401 dance.
